...
void SerialTask( void* params )
{
    ...
    *((BaseType_t*) params) = 1;
    ...
    for( ;; )
    {
	//Handle the events of the queues
	member = xQueueSelectFromSet( queue_set, portMAX_DELAY );
	if( member == queue_from_uart ) //Handle the event of receiving data from the sat
	{
	    
	}
	else if( member == serial_sem )  //Handle the event of receiving a command from the computer
	{
	    
	}
	else
	{
	    Serial.print( "Warning: Invalid set member" );
	}
	// Serial.print( "pog" );
    }
}
